...
#It may not seem like it now, but it is extremely powerful to know all methods
...
###########Rule 3: Matrixes, Dataframes, and Vectors are slightly different
TestData[1,2]
SubSet[1,1] #this will NOT work, wrong dim
...
#They can only hold one kind of data.
class(SubSet) #numeric
#make a matrix from subset
SubSetMat <- cbind(SubSet,SubSet) #short for "col bind"
print(SubSetMat)
...
class(TestData[,1]) #factor <- these are special, I will explain them at another time
...
#In the future, I will teach you how to write functions (it's easy!)
#For now, let's just start playing with existing ones.
...
###Hint: Just do as much as you can with the plot.  If everyone is struggling,
...
colors <- rep("red", length(TestData$num_syllables))
shapes <- rep(19, length(TestData$num_syllables))
colors[greater] <- "blue"
shapes[greater] <- 15
line1 <- lm(TestData$avg_syllable_duration.ms.~TestData$num_syllables)
line2 <- lm(TestData$avg_syllable_duration.ms.[-greater]~TestData$num_syllables[-greater])

plot(TestData$num_syllables, TestData$avg_syllable_duration.ms.,
     col=colors, xlab = "Number of Syllables", ylab="Average Syllable Duration",
     pch=shapes)
abline(line1, lwd = 2)
abline(line2, col="red", lwd = 2)
#optional
legend(x=30,y=95,c("One Uni", "More than One Uni"), pch=c(19,15), col=c("red","blue"))
